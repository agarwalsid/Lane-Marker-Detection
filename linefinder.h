...
// Draw the detected lines on an image
	  void drawDetectedLines2(cv::Mat &image, cv::Scalar color=cv::Scalar(255)) {
		
		  // Draw the lines
		  std::vector<cv::Vec4i>::const_iterator it2= lines.begin();
cv::Point2f fin;
float lin = image.cols;
float lin2 = image.cols;
float liny = 0;
float liny2 = 0;
cv::Point ans1;
cv::Point ans2;
...
if(fin.y > 2*image.rows/3 && fin.y < 5*image.rows/6)
{
if(fin.x< image.cols/2.5)
{
if(lin>image.cols/2.5 - fin.x)
{
lin = image.cols/2.5 - fin.x;
liny = fin.y;
linx = fin.x;
ans1 = pt1;
ans2 = pt2;
...
if(fin.x> image.cols/2.5)
{
if(lin2>fin.x-image.cols/2.5)
{
lin2 = fin.x-image.cols/2.5;
liny2 = fin.y;
linx2 = fin.x;
ans3 = pt1;
ans4 = pt2;
			 ...
			 // cv::line( image, pt1, pt2, color, 6 );
//cv::circle( image, cv::Point(fin.x,fin.y), 4, color, -1, 8, 0 );

		//std::cout<<image.cols/2 << " HoughP line: ("<< pt1 <<"," << pt2 << ")\n"; 
			  ++it2;	
		  }

std::cout<<linx<<","<<linx2<<std::endl;
//cv::circle( image, cv::Point(linx,liny), 4, cv::Scalar(0,255,255), -1, 8, 0 );
//cv::circle( image, cv::Point(linx2,liny2), 4, color, -1, 8, 0 );
 cv::line( image, ans1, ans2, cv::Scalar(0,255,255), 6 );
 cv::line( image, ans3, ans4, cv::Scalar(0,255,255), 6 );
//cv::circle( image, cv::Point(image.cols/2.5,image.rows/2), 4, color, -1, 8, 0 );
	  }
